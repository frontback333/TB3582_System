...
FullData MainWindow::readSensors(){
    ...
    if (!once) {
        ...
        hw.hx.begin(Pins::HX_DT, Pins::HX_SCK);
    ...
    // 2-2) 온도(°C): MAX6675 3채널 사용 예
    {
        ...
        double tComb = hw.readMAX6675C(Pins::SPI_CS_0);
        double tInlt = hw.readMAX6675C(Pins::SPI_CS_1);
        double tCool = hw.readMAX6675C(Pins::SPI_CS_2);
