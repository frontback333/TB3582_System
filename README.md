...
MAX6675는 소프트웨어 SPI로 읽습니다. 공통 MISO는 BCM9(입력), CLK는 BCM11이며 CS는 배기 BCM7 / 입구 BCM8 / 오일 BCM24입니다. BCM25도 비선택 HIGH로 초기화합니다. 변환 간격은 250 ms입니다. 시작 시와 오류 상태가 바뀔 때 CS별 수신 프레임, 단선 비트(D2), 프레임 오류(D15/D1)를 로그로 출력합니다. 읽기 실패는 이전 정상 온도를 유지하지 않고 `nan`으로 표시합니다. CS 이름의 숫자는 하드웨어 SPI CE 번호가 아니므로 실제 BCM 번호로 대조해야 합니다.

ADS1115 초기화 로그에는 버스·주소·채널·실제 sysfs 경로·scale·첫 입력 전압이 표시됩니다. 압력 표시가 0이어도 입력이 0.6 V 미만인지 확인할 수 있습니다. 주소 탐색은 `iio:device0/1` 번호를 고정하지 않고 I2C 주소 경로로 수행합니다.
