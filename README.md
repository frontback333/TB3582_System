...
연료(A1)와 AB(A2)는 0~3.0 V, 오일(A3)은 0~2.73 V를 0~100%로 환산하고 범위를 벗어난 값은 제한합니다. 오일 최대 스로틀에서 약 2.74 V가 측정되어 작은 끝점 여유를 둔 2.73 V로 보정했습니다. AB는 미연결 상태로 확인되었으며, 미연결 ADC 입력은 부유 전압 때문에 0이 아닌 값을 읽을 수 있습니다. 프로그램 시작 시 `0x48`의 A1~A3를 ±4.096 V 범위(`scale=0.125` mV/LSB)로 설정하고 실제 scale을 다시 읽습니다. 기존 ±2.048 V 범위에서는 입력이 포화되어 최대 스로틀이 약 68.26%로 제한됐습니다. 범위 변경에는 해당 IIO scale 파일의 쓰기 권한이 필요하며, 설정 실패 시 해당 채널을 유효한 측정값으로 표시하지 않습니다. 배터리와 압력 채널의 범위는 변경하지 않습니다.
